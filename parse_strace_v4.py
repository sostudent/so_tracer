...
def main():
    print("--- Parser activ (Syscalls, Pipes & Signals Tracking) ---", flush=True)
    ...
    for line in sys.stdin:
        ...
        # --- 6. SIGNALS & TIMERS (New Block) ---
        # Detect Signal Handlers (signal, sigaction, rt_sigaction)
        if "sigaction(" in rest or "signal(" in rest:
            sig_match = re.search(r'(?:sigaction|signal)\((SIG[A-Z0-9_]+)', rest)
            if sig_match:
                log_event(pid, "SIGNAL", f"Handler registered for {sig_match.group(1)}{error_info}", is_error)
            continue
        
        # Detect actual signal delivery (e.g., --- SIGALRM {si_signo=SIGALRM...} ---)
        if rest.startswith("--- SIG"):
            sig_del_match = re.search(r'---\s+(SIG[A-Z0-9_]+)\s+(.*)\s+---', rest)
            if sig_del_match:
                sig_name = sig_del_match.group(1)
                sig_details = sig_del_match.group(2)
                log_event(pid, "SIGRCV", f"Received {sig_name} ({sig_details})")
            continue

        # Detect Alarm System call
        if rest.startswith("alarm("):
            sec = re.search(r'alarm\((\d+)\)', rest)
            if sec:
                log_event(pid, "ALARM", f"Timer set for {sec.group(1)} seconds{error_info}", is_error)
            continue

        # Detect Pause
        if "pause(" in rest:
            # pause() usually returns ? ERESTARTNOHAND on interruption, which isn't an explicit '-1' standard error
            log_event(pid, "PAUSE", "Paused waiting for a signal...")
            continue

        # --- 7. WAIT (New Block) ---
        if re.match(r'^wait', rest): # matches wait, wait4, waitpid
            res = re.search(r'=\s+(-?\d+)', rest)
            if res:
                child_pid = res.group(1)
                if child_pid != "-1":
                    log_event(pid, "WAIT", f"Reaped child PID: {child_pid}")
                else:
                    log_event(pid, "WAIT", f"Wait failed or no children left{error_info}", is_error)
            continue
